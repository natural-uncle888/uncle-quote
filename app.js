...
・拆解後的零件需另行清洗；大樓若洗衣機陽台空間不足，一般會移至浴室／廁所清洗。透天住宅則通常於洗衣機陽台、大門口或浴室／廁所等具備作業與排水空間的位置清洗，實際地點將依現場環境評估。
...
・若現場有門禁、停車或其他進場限制，請提前與我們聯繫。`);
  ...
  const serviceSummary = detectedLabels.length > 0
    ? `本次清洗服務項目：${Array.from(new Set(detectedLabels)).join('、')}`
    : '本次清洗服務項目未符合既有分類，先顯示通用行前提醒。';
